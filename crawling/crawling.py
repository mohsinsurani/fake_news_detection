...
from crawling.crawling_han import Crawling_han
...
def process_data(path_dir, news_dirs):
    ...
    return news_json_arr, tweets_json_arr
...
def process_politifact():
    news_json_arr, tweets_arr = get_data('politifact')
    tweets_arr_han = Crawling_han().get_data_list('politifact')
    save_to_csv(news_json_arr, tweets_arr, "politifact", "tweets_politifact")


def process_gossipicop():
    news_json_arr, tweets_arr = get_data('gossipcop')
    save_to_csv(news_json_arr, tweets_arr, "gossipicop", "tweets_gossipicop")


def get_data(data_cat):
    dataset_directory = sys.path[0] + "/data/FakeNewsNet_Dataset/"
    news_dirs = get_immediate_subdirectories(dataset_directory)
    print(news_dirs)

    politifact_data = list(filter(lambda k: data_cat in k, news_dirs))
    news_json_arr, tweets_arr = process_data(dataset_directory, politifact_data)
    return news_json_arr, tweets_arr


def save_to_csv(news_json_arr, tweets_arr, tweets_han_arr, news_cat, tweets_cat):
    df = pd.DataFrame(news_json_arr, columns=news_json_arr[0].keys())

    tweets_df = pd.DataFrame(tweets_arr, columns=tweets_arr[0].keys())

    tweets_han_df = pd.DataFrame(tweets_han_arr, columns=tweets_han_arr[0].keys())
